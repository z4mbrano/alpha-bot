"""
Sistema de banco de dados multi-usuário com histórico de conversas.
Suporta PostgreSQL (Render) e SQLite (Local/Railway)
Autor: ALPHABOT
Data: 2025-11-19
"""
...
# Detectar ambiente e importar adapter apropriado
USE_POSTGRESQL = bool(os.environ.get('DATABASE_URL') or os.environ.get('POSTGRES_URL'))

if USE_POSTGRESQL:
    print("🐘 Render/PostgreSQL detectado - usando adapter PostgreSQL")
    # Importar todas as funções do PostgreSQL adapter
    from postgresql_adapter import *
    print("✅ Adapter PostgreSQL carregado com sucesso")
else:
    print("🔧 Ambiente local/Railway - usando SQLite")

    # Configuração do DATABASE_PATH apenas para SQLite
    if os.environ.get('RENDER'):
        # Render com SQLite (não recomendado, mas possível)
        DATABASE_PATH = '/opt/render/project/data/alphabot.db'
        data_dir = '/opt/render/project/data'
        if not os.path.exists(data_dir):
            os.makedirs(data_dir, exist_ok=True)
        print(f"🎯 Render SQLite: Usando database em {DATABASE_PATH}")
    ...
    # ========================================
    # IMPLEMENTAÇÃO SQLite ORIGINAL (Fallback)
    # ========================================
    ...
    def save_alphabot_data(user_id: int, dataframe_json: str, metadata: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            cursor.execute(
                '''UPDATE users SET alphabot_data_json = ? 
                   WHERE id = ?''',
            ...
            print(f"❌ Erro ao salvar dados AlphaBot: {e}")
            return False
    ...
    def load_alphabot_data(user_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"❌ Erro ao carregar dados AlphaBot: {e}")
            return None
    ...
    def clear_alphabot_data(user_id: int) -> bool:
        ...
        try:
            ...
            print(f"❌ Erro ao limpar dados AlphaBot: {e}")
            return False
    ...
    def create_alphabot_session(user_id: int, session_id: str, dataframe_json: str, metadata: Dict[str, Any], files_info: List[str]) -> bool:
        ...
        try:
            ...
            # SQLite usa REPLACE para UPSERT
            cursor.execute(
                '''INSERT OR REPLACE INTO alphabot_sessions 
    ...
    def get_alphabot_session(user_id: int, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"❌ Erro ao buscar sessão AlphaBot: {e}")
            return None
    ...
    def get_alphabot_conversation_messages(conversation_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"❌ Erro ao buscar mensagens AlphaBot: {e}")
            return []
    ...
    def delete_alphabot_session(user_id: int, session_id: str) -> bool:
        ...
        try:
            ...
            cursor.execute(
                'DELETE FROM alphabot_sessions WHERE id = ? AND user_id = ?',
                (session_id, user_id)
            )
            success = cursor.rowcount > 0
            conn.commit()
...
if __name__ == '__main__':
    ...
    # Criar usuário de teste apenas se não usar PostgreSQL (para evitar conflito)
